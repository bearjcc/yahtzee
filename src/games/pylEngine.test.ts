...
import { INPUT_SIZE, OUTPUT_SIZE } from '../nn/layout.ts'
...
describe('push-your-luck engines', () => {
  ...
  it('six cubes uses 5000 goal', () => {
    const shape = defaultShape(INPUT_SIZE, OUTPUT_SIZE, 8, 6)
    const genome = randomGenome(shape, mulberry32(5))
    const scratch = createScratch(shape)
    const result = sixCubesGame.playResult(mulberry32(22), (encodeInto) =>
      actFromEncode(encodeInto, genome, shape, scratch),
    )
    expect(result.kind).toBe('pyl')
    if (result.kind !== 'pyl') return
  ...
  it('hot-dice path can continue after scoring all six', () => {
    // Bank immediately whenever possible: still must terminate.
    const result = playPyl(
      mulberry32(99),
      () => {
        const out = new Float32Array(OUTPUT_SIZE)
        for (let i = 0; i < 6; i++) out[i] = 1
        out[6] = 1 // bank
        return out
      },
      { mode: 'farkle', goal: 500, oneHotIndex: 1, maxTurns: 80 },
    )
