...
DiceLab (`package.json` name `yahtzee`) is a single, zero-backend Vite + TypeScript SPA that trains neural nets to play Yahtzee entirely in the browser (Web Workers + IndexedDB). There is no server, database, API, or secrets to configure.
